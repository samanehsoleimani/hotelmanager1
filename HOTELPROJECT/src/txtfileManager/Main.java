...
public class Main extends JFrame {
	...
	public Main() {
		...
		tabs.setTabPlacement(JTabbedPane.LEFT);  // برای نمایش تب‌ها به صورت عمودی در سمت چپ
		// تغییر اندازه تب‌ها
        ...
        tabs.addTab("غذا", createFoodPanel());
        tabs.addTab("هزینه اتاق ها ", createRoomBooking());
        tabs.addTab("لیست کارکنان", createEmployees());
        tabs.addTab("تفریحات", entertainmentPanel());
        tabs.addTab("راه های ارتباطی", connectionPanel());
		tabs.addTab("مدیریت کارمندان", createEmployeePanel());
        tabs.addTab("انبار", createProductPanel());
        tabs.addTab("دوره کارکنان", createRegistrationPanel());
        tabs.addTab("حقوق", createPayrollPanel());        
        tabs.addTab("کنفرانس", createConferencePanel());
	...
	private JPanel createGuestPanel(){
		...
		addGuest.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(panel, "⚠ لطفاً اطلاعات را صحیح وارد کنید", "خطا",
	...
	private JPanel creatsurvaypanel() {
		...
		showSurveys.addActionListener(e -> {
			// Use SwingUtilities.invokeLater to ensure updates happen on the Event Dispatch
			// Thread
	...
	private JPanel createEmployeePanel() {
		...
		addEmployee.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(panel, "⚠ لطفاً اطلاعات عددی را صحیح وارد کنید", "خطا",
						JOptionPane.ERROR_MESSAGE);
			}
		});
	...
	private JPanel createProductPanel() {
		...
		addProduct.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(panel, "⚠ لطفاً اعداد را به درستی وارد کنید", "خطا",
						JOptionPane.ERROR_MESSAGE);
			} catch (IllegalArgumentException ex) {
				JOptionPane.showMessageDialog(panel, "⚠ " + ex.getMessage(), "خطا", JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		updateQuantity.addActionListener(e -> {
			...
			if (name == null || name.trim().isEmpty()) {
				JOptionPane.showMessageDialog(panel, "⚠ نام کالا نمی‌تواند خالی باشد");
				return;
			}
			...
			if (found == null) {
				JOptionPane.showMessageDialog(panel, "⛔ کالایی با این نام یافت نشد.");
				return;
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(panel, "⚠ لطفاً یک عدد معتبر وارد کنید", "خطا",
						JOptionPane.ERROR_MESSAGE);
			}
		});
	...
	private JPanel createRegistrationPanel() {
		...
		for (int i = 0; i < Constants.COURSES.length; i++) {
			for (int j = 0; j < Constants.COURSES[i].length; j++) {
				...
				courseCombo.addItem(String.format("%s | %s | %s (%s تومان)", course[0], course[2], course[3],
		...
		registerBtn.addActionListener(e -> {
			...
			if (!nationalId.matches("\\d{10}")) {
				JOptionPane.showMessageDialog(panel, "⚠ کد ملی نامعتبر است!", "خطا", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			Constants.Registration reg = new Constants.Registration(fullName, nationalId, phone, course[0],
					Integer.parseInt(course[1]), course[2], course[3]);
	...
	private JPanel createPayrollPanel() {
		...
		formPanel.add(new JLabel("⏱ ساعات کاری:"));
		...
		generateButton.addActionListener(e -> {
			try {
				EmployeePayroll payroll = new EmployeePayroll(nameField.getText().trim(), jobField.getText().trim(),
						dateField.getText().trim(), Double.parseDouble(baseSalaryField.getText().trim()));
				...
				JOptionPane.showMessageDialog(panel, "⚠ لطفاً مقادیر عددی را صحیح وارد کنید", "خطا",
						JOptionPane.ERROR_MESSAGE);
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(panel, "⚠ خطا: " + ex.getMessage(), "خطا", JOptionPane.ERROR_MESSAGE);
			}
		});
	...
	private JPanel createConferencePanel() {
		...
		formPanel.add(new JLabel("🏷️ نوع رویداد:"));
		...
		createButton.addActionListener(e -> {
			try {
				...
				if (reserved) {
					...
					outputArea.setText("⚠ امکان رزرو سالن نیست.");
				}
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(panel, "⚠ خطا: " + ex.getMessage(), "خطا", JOptionPane.ERROR_MESSAGE);
			}
